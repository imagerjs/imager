...
Imager.prototype = {
  ...
  prepareUpload: function (file, filename, variant, fn) {
    ...
    if (variant.transparentBG) {
      Object.keys(variant.transparentBG).forEach(function (name) {
        var processFiles = function (cb) {
          var preset = {
            name: name,
            color: variant.transparentBG[name],
            sep: variant.separator || '_'
          };
          self.transparentBG(file, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
  ...
  /**
   * Convert to transparent background image
   *
   * @param {Object} file
   * @param {Object} preset
   * @param {String} filename
  ...
  transparentBG: function(file, preset, filename, cb) {
    var self = this;
    var ct = file['type'] || file.headers['content-type'];
    var remoteFile = preset.name + preset.sep + filename;
    var tempFile = path.join(tempDir, 'imager_' +
